...
description: "couple-calendar 프로젝트의 풀스택 기능 개발 오케스트레이터. 백엔드(Kotlin/Spring Boot), 웹(Next.js Clean Architecture), 통합 QA를 조율. 새 기능 추가, API 엔드포인트 설계, 웹 도메인/유스케이스/DTO 구현, 커플/이벤트/사용자/인증 도메인 작업 시 사용. 후속 작업: 기능 수정, 재실행, 업데이트, 보완, 부분 재구현, 이전 결과 개선, 정합성 재검증, QA 재실행, '웹', 'web', 'apps/web', '클린 아키텍처', 'couple', 'calendar', '이벤트', '커플', '캘린더', 'API', '프론트' 관련 요청 시에도 반드시 이 스킬을 사용할 것."
---

# Couple Calendar Development Orchestrator

couple-calendar 프로젝트의 풀스택 기능 개발을 조율한다. 백엔드 API, 웹 앱, 통합 QA를 수행하여 경계면 정합성이 보장된 기능을 산출한다.

## 실행 모드: 서브 에이전트

**모드 선택 이유:**
- 백엔드(Kotlin), 웹(Next.js/Clean Arch)은 완전히 독립된 코드베이스
- 개발 중 실시간 통신이 구조적으로 불필요 — 각자의 산출물만 명확하면 됨
- QA는 두 축 산출물을 읽고 검증하는 단방향 작업
- 팀 통신 오버헤드보다 결과 전달 효율이 더 적합한 3인 구성 (전문성 축별 1명)
...
**기본 전략: 순차 실행 (Phase 2-B)** — 스펙 정합성 우선. 백엔드 스펙이 확정된 후 프론트(웹)를 실행한다.

#### Phase 2-A: backend-dev (필요 시, 포그라운드)
영향 범위에 백엔드가 포함되면 실행. 출력: `_workspace/02_backend_api_spec.md` + Kotlin 소스.

#### Phase 2-B: web-dev 실행 (포그라운드)

| 에이전트 | 입력 | 출력 | run_in_background |
|---------|------|------|-------------------|
| web-dev | `_workspace/01_requirements.md`, `_workspace/02_backend_api_spec.md`, `docs/plans/2026-04-20-four-week-web-clean-architecture.md` | `_workspace/02_web_api_integration.md` + TS/TSX 소스 | false |

**의존성 규칙:**
- 백엔드 스펙이 확정되지 않은 상태로 web-dev를 실행하지 않는다
- 백엔드 변경이 없는 작업이면 Phase 2-A를 건너뛰고 바로 2-B 진행
- web-dev는 단일 포그라운드로 실행한다 (run_in_background=false)
...
- 실제 Kotlin/TypeScript 소스 교차 비교 (API↔웹 단일 축)
...
QA 보고서의 FAIL 항목별로:
1. 수정 대상 에이전트 식별 (backend-dev / web-dev / 복수)
...
[오케스트레이터]
    ...
    │                                   │
    │                                   ↓ (읽기)
    ├── Phase 2-B ──────────────→ Agent(web-dev, model=opus)
    │                                   │
...
| 백엔드 스펙 미확정 | Phase 2-B 순차 실행 (web-dev는 backend-dev 후) |
| 타임아웃 | 현재까지 수집된 부분 결과 사용, 사용자에게 상태 보고 |

## 팀 크기 가이드

중규모 (10-20개 작업): 3명 — 현재 구성 적합 (backend/web/qa).
DB 마이그레이션·배포·디자인 전문가가 필요해지면 추가 고려.

## 테스트 시나리오

### 정상 흐름 1: 풀스택 이벤트 "알림" 기능 추가
1. 사용자 입력: "이벤트에 알림 기능 추가. 시작 N분 전 알림."
2. Phase 1: 영향 범위 = 백엔드 + 웹
3. Phase 2-A: backend-dev → Event에 `reminderMinutes: Int?` 추가, spec 문서화
4. Phase 2-B: web-dev → EventDto + Event entity + useEventForm UseCase 수정
5. Phase 3: qa-integrator 2-way(API↔Web) 검증 → PASS
6. Phase 4: 최종 보고

### 정상 흐름 2: 웹 단독 작업 — 도메인 신규 구현
1. 사용자 입력: "웹에 anniversary 도메인 구현 (Week 3 Task 3.4)"
2. Phase 1: 영향 범위 = 웹 단독 (백엔드는 이미 완성)
