...
3. `grep -rE "fetch\\(|localStorage" apps/web/src/domains/*/domain/` 결과가 비어 있어야 함
4. `yarn workspace @couple-calendar/web check:architecture` 실행 결과가 PASS
5. DTO(`data/dto/`)가 백엔드 Response와 필드명 1:1 일치하는지 확인 (필드명 번역은 Mapper에서만)
6. Entity가 DTO 필드와 다르다면 Mapper에 대응 변환이 있는지 확인

위반 발견 시 FAIL로 보고하고 web-dev에게 수정 지시.
