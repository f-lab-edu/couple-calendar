 ...
 *
 * All icons here are purely decorative and marked `aria-hidden`; the labelled
 * control (button/link) that wraps them carries the accessible name. Biome's
 * noSvgWithoutTitle does not recognize aria-hidden, so it is suppressed for the file.
 */
// biome-ignore-all lint/a11y/noSvgWithoutTitle: decorative icons; parent control provides the a11y name (aria-hidden set on each svg)
...
export function AppleIcon({ s = 20, style, className }: IconProps) {
	return (
		<svg width={s} height={s} viewBox="0 0 24 24" fill="currentColor" style={style} className={className} aria-hidden>
